...
public class Maze implements Serializable {
	...
	private ArrayList<Tile> unvisitedTiles;
	...
	private void initialiseUnvisitedList()
	{
		// loop through the object grid, if a tile is found, add it to the unvisited list
		for (MazeObject[] row : getObjectGrid())
		{
			for (MazeObject obj : row)
			{
				if (obj instanceof Tile)
				{
					getUnvisitedTiles().add((Tile) obj);
				}
			}
		}
	}
	
	public ArrayList<Tile> getUnvisitedTiles()
	{
		if (unvisitedTiles == null)
		{
			unvisitedTiles = new ArrayList<Tile>();
		}
		return unvisitedTiles;
	}
	
	// called whenever a tile has been visited, so it can be removed from the unvisited tiles list
	public void tileVisited(Tile tile)
	{
		if (getUnvisitedTiles().contains(tile))
		{
			getUnvisitedTiles().remove(tile);
		}
	}
